    ...
    function findPurchasedStocksList($data) {
        ...
        if (!empty($stock_list)) {
            foreach ($stock_list as $data) {
                ...
                $getGainAndLossValues = $this->UserstockHistory->getShareWorthUserStocks($data['UserStock']['portfolio_id'], $data['Share']['share_id']);
                
                $data['Share']['average_share_price'] = $getGainAndLossValues[$data['Share']['share_id']];
                $data['Share']['average_share_percentage'] =  $getGainAndLossValues["share_id_".$data['Share']['share_id']."_percentage"];
