        ...
        if ($paidGamesIds) {
            ...
            if ($dayTradePortfolioList) {
                foreach ($dayTradePortfolioList as $key => $val) {
                    ...
                    $this->UserstockHistory->deleteAll(array('UserstockHistory.portfolio_id' => $val['Portfolio']['id']), true);
                }
                return true;
            } else {
                return false;
...
//            $game_id = $portfolioData['Portfolio']['game_id'];
//            switch ($game_id) {
//                case 1:
//                    $this->Watchlist->deleteAllDayTradeWatchlist($data);
//                    break;
//                case 2:
//                    $this->Watchlist->deleteAllDefaultProfolioWatchlist($data);
//                    break;
//            }
     ...
     * @author Ganesh
     * @copyright Softway solutions
     * @uses This function is used for return the user list who are all playing daytrade game 
     */
    public function getDayTradeUsers() {
        $conditions = array("Portfolio.game_id" => 2);
     ...
     * @author Ganesh
     * @copyright Softway solutions
     * @uses This function is used to send the push notification to the user when daytrade 
     * game finish 
     */
    function sendDaytradeNotifications($push_token_array, $push_msg) {
        foreach ($push_token_array as $token) {
            if (!empty($token["UserLog"]["push_note_token"])) {
                App::import('Component', 'ApiComponent');
                $apiComponent = new ApiComponent();
                $deviceToken = $token["UserLog"]["push_note_token"];
                $apiComponent->send_ios_notification_daytrade($deviceToken, $push_msg);
            }
        }
    }
    
    function isDefaultPortfolio($portfolio_id, $user_id){
        
        $conditions = array("Portfolio.game_id"=>1, "Portfolio.user_id" => $user_id);
        $orderby = array("Portfolio.created");
        $fields = array("Portfolio.id");
        $values = $this->find("all", array('conditions' => $conditions, 'fields' => $fields, 'order' => $orderby, 'limit' => 1, 'recursive' => -1));
        
        if($values && $values[0]["Portfolio"]["id"] == $portfolio_id){            
            return true;
        }else{            
            return false;
        }
    ...
    function getDefaultPortfolioByUserId($user_id){
        ...
        if(count($values)>0){
            $default_portfolio_id =$values[0]["Portfolio"]["id"];
        }
    ...
    function getDayTradePortfolioByUserId($user_id){
       
        $conditions = array("Portfolio.game_id"=>2, "Portfolio.user_id"=>$user_id);
        $fields = array("Portfolio.id");
        $values = $this->find("all", array('conditions' => $conditions, 'fields' => $fields, 'recursive' => -1));
        
        $daytrade_portfolio_id = 0;
        if(count($values)>0){
            $daytrade_portfolio_id =$values[0]["Portfolio"]["id"];
        }
        
        return $daytrade_portfolio_id;
        
    }
    
//    function getDefaultPortfolioByUser(){
//        
//        $conditions = array("Portfolio.game_id"=>1);
//        $groupby = array("Portfolio.user_id");
//        $orderby = array("Portfolio.created");
//        $fields = array("Portfolio.id", "Portfolio.user_id");
//        $values = $this->find("all", array('conditions' => $conditions, 'fields' => $fields, 'order' => $orderby, 'group' => $groupby, 'recursive' => -1));
//    
//        return $values;
//    }
//    
//    function findDayTradeGameByUserId($user_id = ""){
//        if($user_id == 0){
//            $conditions = array("Portfolio.game_id"=>2);
//        }else{
//            $conditions = array("Portfolio.game_id"=>2, "Portfolio.user_id" => $user_id);
