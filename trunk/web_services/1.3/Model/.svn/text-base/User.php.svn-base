...
class User extends AppModel {
    ...
    public function finalPortfolioList($data) {
        ...
        if (!empty($portfoliolistData)) {
            foreach ($portfoliolistData as $data) {
			  ...
			  $data['p']['portfolio_percentage_change'] = ($data['p']['portfolio_worth'] - $data['p']['start_money'])*100/$data['p']['start_money'];
                $response[] = $data['p'];
            }
        }
        if ($data) {
     ...
     * with portfolio_worth and total count of stocks 
     */
    public function allPortfolioList($data) {
        $portfoliolistData = $this->query('select  
                p.id as portfolio_id, p.portfolio_name, p.net_value as pnet_value, p.trades as available_trades, p.available_cash, u.portfolio_stock_count,(COALESCE(p.available_cash,0) + COALESCE(u.total,0)) as portfolio_worth, p.start_money, p.created 
                FROM
                (
        ...
        $response = array();
        if (!empty($portfoliolistData)) {
            foreach ($portfoliolistData as $data1) {
                $data1['p']['portfolio_stock_count'] = strval(intval($data1['u']['portfolio_stock_count']));
                $data1['p']['portfolio_worth'] = $data1['0']['portfolio_worth'];
                $data1['p']['net_value_change'] = ($data1['p']['portfolio_worth'] - $data1['p']['start_money']);
				$data1['p']['type']='Not-Default';
				$portfolioId=$this->Portfolio->getDefaultPortfolioByUserId($data['user_id']);
				if($portfolioId==$data1['p']['portfolio_id']){
				$data1['p']['type']='Default';
				}
				$portfolioId=$this->Portfolio->getDayTradePortfolioByUserId($data['user_id']);
				if($portfolioId==$data1['p']['portfolio_id']){
				$data1['p']['type']='Default';
				}
                unset($data1['u']);
                ...
                $todays_date = date('Y-m-d h:i:s');
               // $data['p']['portfolio_percentage_change'] = $this->UserstockHistory->getPortfolioWorth($data['p']['portfolio_id'], $data['p']['created'], $todays_date);
			  $data1['p']['portfolio_percentage_change'] = ($data1['p']['portfolio_worth'] - $data1['p']['start_money'])*100/$data1['p']['start_money'];
                $response[] = $data1['p'];
            }
        }
	else{
