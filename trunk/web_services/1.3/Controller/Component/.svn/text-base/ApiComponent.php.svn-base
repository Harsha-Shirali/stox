...
//    }
    ...
    function send_ios_notification($deviceToken, $msg) {
        ...
        $apnsPort = 2195;
        // PEM file location
        $apnsCert = WWW_ROOT . 'files/pemfiles/STOX-PROD.pem';
    ...
    function splash_screen($data) {
        if ($this->UserLog->checkAccessTokenValid($data)) {
			...
			$body["indices"] = array($this->Share->getMarketShare1(),$this->Share->getMarketShare2(),$this->Share->getMarketShare3());
            $status = TRUE;
     ...
     * @copyright Softway solutions
     * @param array $data
     * @uses This function is used to get market share data and to check share is watched
     */
	  function market_share_detail($data) {
        if ($this->UserLog->checkAccessTokenValid($data)) {
            $shareData = $this->Share->marketShareDetail($data);
			$data['portfolio_id'] = $this->Portfolio->getDefaultPortfolioByUserId($data['user_id']);
			$shareExistsInWatchlist = $this->Watchlist->checkShareExistsInWatchlist($data);
			
			if(!empty($shareExistsInWatchlist)){
				$shareExistsInWatchlist['watchlist_exists']=TRUE;			
				}
			else{
				$shareExistsInWatchlist['watchlist_exists']=FALSE;	
			}
            if ($shareExistsInWatchlist) {
                $body = array_merge($shareData,$shareExistsInWatchlist);
                $status = True;
                $message = True;
            } else {
                $body = null;
     ...
     * web service method
     */
    function all_portfolio_list($data) {
        if ($this->UserLog->checkAccessTokenValid($data)) {
            $getPortfolioList = $this->User->allPortfolioList($data);
            $body = $getPortfolioList;
            $status = TRUE;
            $message = Configure::read('site.show_portfolio_list_message1');
        } else {
