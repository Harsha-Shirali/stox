...
class User extends AppModel
{
	...
	/*method commented as it relates to guest user part
	 * 
...
(
  ...
  group by portfolio_id
	...
	//        function getUserCreatedDatetime($data){
	//            $created_datetime = $this->find('first', array('conditions' =>
	// array('User.id' => $data["user_id"]), 'fields' => array('User.created'),
	// 'recursive' => -1));
	//            if ($created_datetime) {
