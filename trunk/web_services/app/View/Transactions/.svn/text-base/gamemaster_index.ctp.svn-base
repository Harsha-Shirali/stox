<div class="transactions index content-with-left-menu">
	...
	<tr>
		...
		<td>
                    <?php if ($transaction['Transaction']['created'] != 0) { ?>
                        <?php echo h(date('M d, Y H:i:s', strtotime($transaction['Transaction']['created']))); ?>
                    <?php } else { ?>
                        ---
                    <?php } ?>
                    &nbsp;
		...
		<td>
                    <?php if ($transaction['Transaction']['modified'] != 0) { ?>
                        <?php echo h(date('M d, Y H:i:s', strtotime($transaction['Transaction']['modified']))); ?>
                    <?php } else { ?>
                        ---
                    <?php } ?>
                    &nbsp;
		...
		<td class="actions">
			...
			<?php echo $this->Form->postLink(__(''), array('action' => 'delete', $transaction['Transaction']['id']),array('class'=>'delete-icon','title'=>'Delete'), __('Are you sure you want to delete # %s?', $transaction['Transaction']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
